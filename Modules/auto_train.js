class AutoTrain extends ModernUtil {
    // === Constants =============================================================
    static POWER_LIST   = ['call_of_the_ocean', 'spartan_training', 'fertility_improvement'];
    static GROUND_ORDER = ['catapult', 'sword', 'archer', 'hoplite', 'slinger', 'rider', 'chariot'];
    static NAVAL_ORDER  = ['small_transporter', 'bireme', 'trireme', 'attack_ship',
                           'big_transporter', 'demolition_ship', 'colonize_ship'];
    ...
    static TROOP_BG = {
        ...
        chariot:           [200, 100],
        catapult:          [150, 150],
        big_transporter:   [  0, 150],
        small_transporter: [300, 350],
        bireme:            [ 50, 150],
        demolition_ship:   [250,   0],
        attack_ship:       [150, 100],
        trireme:           [400, 250],
        colonize_ship:     [ 50, 200],
    };
    ...
    constructor(c, s) {
        ...
        this.interval             = setInterval(this.main.bind(this), this._randomDelay(1000, 10000));
    ...
    settings = () => {
        requestAnimationFrame(() => {
            ...
            uw.$('#troops_lvl_buttons').on('mousedown', e => { this.shiftHeld = e.shiftKey; });
        });
        ...
        <div class="game_border">
            ...
            <div class="game_border_corner corner1"></div><div class="game_border_corner corner2"></div>
            <div class="game_border_corner corner3"></div><div class="game_border_corner corner4"></div>
            <div id="auto_train_title" class="game_header bold"
                 style="position: relative; cursor: pointer"
                 onclick="window.modernBot.autoTrain.trigger()">
                <span style="z-index: 10; position: relative;">Auto Train </span>
                <div style="position: absolute; right: 10px; top: 4px; font-size: 10px; z-index: 10">
                    (click to reset)
                </div>
                <span class="command_count"></span></div>
    ...
    handleSpell = (e) => {
        e = !!e;
        if (this.spell !== e) { this.spell = e; this.storage.save('at_spell', e); }
        $('#train_passive').toggleClass('disabled', e);
        $('#train_spell').toggleClass('disabled', !e);
    };
    ...
    handlePercentual = (n) => {
        ...
        if (this.percentual !== n) { this.percentual = n; this.storage.save('at_per', n); }
    };

    // === Population accounting =================================================
    // NOTE: preserves original formula using units_left for partial train orders
    getTotalPopulation = (town_id) => {
        ...
        for (let order of orders) {
            used += data[order.attributes.unit_type].population
                  * (order.attributes.units_left / order.attributes.count)
                  * order.attributes.count;
        }
    ...
    setPolisInSettings = (town_id) => {
        ...
        const getTroopHtml = (troop) => {
            const bg = AutoTrain.TROOP_BG[troop] ?? [0, 0];
            let color = 'red';
            if (isGray(troop)) {
                return ` <div class="auto_build_box">
                    <div class="item_icon auto_trade_troop"
                         style="background-position: -${bg[0]}px -${bg[1]}px; filter: grayscale(1);">
                    </div>
                </div> `;
            }
            return ` <div class="auto_build_box">
                <div class="item_icon auto_trade_troop"
                     onclick="window.modernBot.autoTrain.editTroopCount(${town_id}, '${troop}', 0)"
                     style="background-position: -${bg[0]}px -${bg[1]}px; cursor: pointer">
                    <div class="auto_build_up_arrow"
                         onclick="event.stopPropagation(); window.modernBot.autoTrain.editTroopCount(${town_id}, '${troop}', 1)">
                    </div>
        ...
        const allTroops = [...AutoTrain.GROUND_ORDER, ...AutoTrain.NAVAL_ORDER];

        uw.$('#troops_lvl_buttons').html(` <div id="troops_settings_${town_id}">
            ...
            <div style="width: 831px; display: inline-flex; gap: 1px;">
                ${allTroops.map(t => getTroopHtml(t)).join('')}
            </div>
        </div>`);
    };
    ...
    editTroopCount = (town_id, troop, count) => {
        ...
        if (count) {
            ...
            count = this.shiftHeld ? count * AutoTrain.SHIFT_LEVELS[troop][index] : count;
        } else {
            count = 10000;
        }
        ...
        if (total_pop - used_pop < unit_pop * count) {
            count = parseInt((total_pop - used_pop) / unit_pop);
        }
    ...
    updatePolisInSettings = (town_id) => {
        ...
        uw.$('#auto_train_title').css('filter', isTownActive ? AutoTrain.ACTIVE_FILTER : '');
    };
    ...
    trigger = () => {
        ...
        if (this.city_troops[town_id]) {
            ...
            [...AutoTrain.NAVAL_ORDER, ...AutoTrain.GROUND_ORDER].forEach(troop => {
                const selector = `#troops_settings_${town_id} #troop_lvl_${troop}`;
    ...
    getTroopCount = (troop, town_id) => {
        ...
        let duable_with_pop = parseInt(resources.population / uw.GameData.units[troop].population);
        ...
        let max   = parseInt(Math.min(w_max, s_max, i_max) * 0.85);
